 ...
 *
 * <div class="preformat">{@code DQ_EvaluationMethod}
...
@XmlRootElement(name = "DQ_EvalutionMethod")
@XmlSeeAlso({
...
public class DefaultEvaluationMethod extends ISOMetadata implements EvaluationMethod{
     ...
     */
    private Collection<Citation> referenceDoc;
     ...
     */
    public static final class Dates extends AbstractList<Date>
    ...
    public DefaultEvaluationMethod(final EvaluationMethod object) {
        ...
        if (object != null) {
            ...
            referenceDoc                = copyCollection(object.getReferenceDoc(), Citation.class);
     ...
     *   <li>Otherwise if the given object is an instance of {@link AggregationDerivation} or
     *       {@link DataEvaluation}, then this method delegates to
    ...
    public EvaluationMethodType getEvaluationMethodType() {
        return FilterByVersion.CURRENT_METADATA.accept() ? evaluationMethodType : null;
    }
    ...
    public InternationalString getEvaluationMethodDescription() {
        return FilterByVersion.CURRENT_METADATA.accept() ? evaluationMethodDescription : null;
    }
    ...
    public Citation getEvaluationProcedure() {
        return FilterByVersion.CURRENT_METADATA.accept() ? evaluationProcedure : null;
    }
    ...
    public Collection<Citation> getReferenceDoc() {
        if (!FilterByVersion.CURRENT_METADATA.accept()) return null;
        return referenceDoc = nonNullCollection(referenceDoc, Citation.class);
    }

    /**
     * Sets the information on documents which are referenced in developing and applying a data quality evaluation method.
     *
     * @param  newValues  the new name of measures.
     */
    public void setReferenceDoc(final Collection<? extends Citation> newValues) {
        referenceDoc = writeCollection(newValues, referenceDoc, Citation.class);
    }
    ...
    public Collection<Date> getDates() {
        if (Semaphores.query(Semaphores.NULL_COLLECTION)) {
            return isNullOrEmpty(dateTime) ? null : dateTime;
        }
        if (dateTime == null) {
            dateTime = new DefaultEvaluationMethod.Dates();
        }
        return dateTime;
    }
    ...
    public void setDates(final Collection<? extends Date> newValues) {
        if (newValues != dateTime) {               // Mandatory check for avoiding the call to 'dates.clear()'.
            checkWritePermission(valueIfDefined(dateTime));
