...
public final class TiledRasterResource extends TiledGridCoverageResource implements StoreResource {
    ...
    /**
     * Path to the source file for information purpose, or {@code null} if unknown.
     *
     * @see #getFileSet()
     */
    ...
    public static List<Resource> create(final Decoder decoder, final DataStore lock) throws IOException, DataStoreException {
        ...
        final var byName = new HashMap<GenericName,List<TiledRasterResource>>(); // For detecting name collisions.
        ...
        for (int i = 0; i < variables.length; i++) {
                ...
                for (final String keyword : VECTOR_COMPONENT_NAMES) {
                    ...
                    if (prefixLength >= 0) {
                        ...
                        for (int j=i; ++j < variables.length;) {
                            ...
                            {
                                // Check if the tiles are compatible between the two variables (same tilling)
            ...
            final var r = new TiledRasterResource(decoder, name.trim(), grid, siblings, numBands, bandDimension, lock);
        ...
        do {
            changed = false;
            List<TiledRasterResource> collisions = null;
            for (final Iterator<List<TiledRasterResource>> it = byName.values().iterator(); it.hasNext();) {
                final List<TiledRasterResource> rs = it.next();
                if (rs.size() >= 2) {
            ...
            if (collisions != null) {                       // After above loop because may change the byName` map.
                for (final TiledRasterResource r : collisions) {
     ...
     */
    private void addToNameMap(final Map<GenericName,List<TiledRasterResource>> byName) {
    ...
    public List<Variable> getVariables() {
        return Containers.viewAsUnmodifiableList(data);
    }
    ...
    protected SampleModel getSampleModel(final int[] bands) throws DataStoreContentException {
        ...
        if (sampleModel == null) {
            final Variable visbleBand = data[visibleBand];
            final DataType dataType = visbleBand.getDataType();

            RuntimeException error = null;
            final org.apache.sis.image.DataType type = dataType.rasterDataType;
            if (type != null) try {
                int[] tileSize = getTileSize();
                var size = new java.awt.Dimension(tileSize[0], tileSize[1]);
                var numBits = new int[sampleDimensions.length];
                Arrays.fill(numBits, type.size());
                sampleModel = new SampleModelBuilder(type, size, numBits, true).build();
            } catch (IllegalArgumentException | RasterFormatException e) {
                error = e;
            }
            if (sampleModel == null) {
                throw new DataStoreContentException(Errors.format(Errors.Keys.UnsupportedType_1, "Error while creating the SampleModel"), error);
            }
        }
        return sampleModel;
    }
    ...
    protected ColorModel getColorModel(final int[] bands) throws DataStoreContentException {
        ...
        if (colorModel == null) {
            ...
            Function<Category, Color[]> colors = main.decoder.convention().getColors(main);
            if (colors != null) {
                colors = ColorScaleBuilder.GRAYSCALE;
            }

            final var colorizer = new ColorScaleBuilder(colors, null, false);
            final var sm = getSampleModel(null);
            if (colorizer.initialize(sm, visibleBand)) {
                org.apache.sis.image.DataType type = org.apache.sis.image.DataType.forDataBufferType(sm.getDataType());
                colorModel = colorizer.createColorModel(type, data.length, visibleBand);
            } else {
                colorModel = ColorScaleBuilder.NULL_COLOR_MODEL;
            }
        }
        return colorModel;
    }
    ...
    protected final TiledGridCoverage read(final Subset subset) throws DataStoreException {
        return new TiledRaster(this, subset);
